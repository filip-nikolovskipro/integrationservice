node('docker') {
    ...
    //sh "docker build -t integrationservice:test-B${BUILD_NUMBER} -f Dockerfile.Integration ."
    
    stage 'Pusblish integration service'
        
        containerID = sh (
            script: "docker run -d integrationservice:B${BUILD_NUMBER}", 
        returnStdout: true
        ).trim()
